...
def psthAreaFeature(spikeData, binWidth=50, smoothWindow=3):
    psth, _, binDuration = getPSTHAndFano(
        spikeData,
        binWidth=binWidth,
        smoothWindow=smoothWindow
    )
...
def plotFeatureResultsBarChart(results):
    ...
    for featureName in featureOrder:
        ...
        testAccuracies.append(stats["overallAccuracy"])
        rampAccuracies.append(1 - stats["rampErrorRate"])
        stepAccuracies.append(1 - stats["stepErrorRate"])
    ...
    plt.ylabel("Accuracy")
    plt.ylim(0, 1)
    plt.title("Ad-hoc feature classification accuracy")          #Single-feature accuracies of classifiying
    plt.legend()
    plt.tight_layout()
    plt.savefig(r"Figure for Latex/Week 3/FigureName3.pdf", bbox_inches="tight")
    plt.show()
