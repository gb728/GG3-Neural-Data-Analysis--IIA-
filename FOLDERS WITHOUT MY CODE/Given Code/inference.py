...
# Added so its quicker by using more cores of cpu
@numba.jit(nopython=True, parallel=True, cache=True)
def batch_hmm_normalizer(pi0, Ps, lls):
    """
    pi0: shape (K,)
    Ps: shape (1, K, K) or (T-1, K, K)
    lls: shape (Ntrials, T, K)
    """
    Ntrials = lls.shape[0]
    T = lls.shape[1]
    K = lls.shape[2]

    normalizers = np.zeros(Ntrials)

    for n in numba.prange(Ntrials):
        alphas = np.zeros((T, K))
        normalizers[n] = forward_pass(pi0, Ps, lls[n], alphas)
