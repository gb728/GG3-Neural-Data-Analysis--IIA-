...
def gamma_isi_point_process(rate, shape):
    """
    Simulates (1 trial of) a sub-poisson point process (with underdispersed inter-spike intervals relative to Poisson)          # Basically a less random way of getting the spikes than the poisson process of sampling
...
class StepModel():
    ...
    def __init__(self, m=50, r=10, x0=0.2, Rh=50, isi_gamma_shape=None, Rl=None, dt=None):
        """
        Simulator of the Stepping Model of Latimer et al. Science 2015.
        :param m: mean jump time (in # of time-steps). This is the mean parameter of the Negative Binomial distribution                 #mean value of time(bin) that the spike will jump at
                  of jump (stepping) time
        :param r: parameter r ("# of successes") of the Negative Binomial (NB) distribution of jump (stepping) time                   # r is the variability of jump time (variance basically)
    ...
    def emit(self, rate):                   # I think this function is just here so it gets used in the simulate function ygm
        """
    ...
    def simulate(self, Ntrials=1, T=100, get_rate=True):
        ...
        for tr in range(Ntrials):
            ...
            rate = np.ones(T) * self.x0 * self.Rh                       #x0 times Rh = R0!!! (which is like the original baseline rate!)
...
class RampModel():
    ...
    @property
    def params(self):
        return self.beta, self.sigma, self.x0   # me when bug in the code!! ( Just a typo). Orginally it wrote ' return self.mu, self.sigma, self.x0 ' , but self.mu was changed to self.beta
    ...
    def simulate(self, Ntrials=1, T=100, get_rate=True):
       ...
       # simulate all trials in parallel (using numpy arrays and broadcasting)
        ...
        ts = np.arange(T)
        xs = self.x0 + self.beta * dt * ts + self.sigma * np.sqrt(dt) * np.cumsum(npr.randn(Ntrials, T), axis=1)                    # literally just implementation of starting the iteration of the drift diffusion
...
#=========================================
""" Just testing out the functions and seeing what they do/how they work"""

#print('done')
#ramp = RampModel()
#spikesramp, xs, ratesramp = ramp.simulate()
#print(f"spikesramp: {spikesramp}")
#print(f"xs: {xs}")
#print(f"ratesramp: {ratesramp}")
#step = StepModel()
#print(step.params)

#spikes , jumps , rates = step.simulate()
